  ...
  AssetHelpers
{
    ...
    )
  {
    ...
    require(
      spendAssets_.isUniqueSet(),
      "__preProcessCoI: Duplicate spend asset"
    );
    require(
    ...
    for (uint256 i; i < incomingAssets_.length; i++) {
      ...
      preCallIncomingAssetBalances_[i] = ERC20(incomingAssets_[i]).balanceOf(
        _vaultProxy
      );
    }
    ...
    )
  {
    ...
    for (uint256 i; i < _incomingAssets.length; i++) {
      incomingAssetAmounts_[i] = __getVaultAssetBalance(
        _vaultProxy,
        _incomingAssets[i]

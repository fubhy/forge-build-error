...
abstract contract IdleV4ActionsMixin is AssetHelpers {
    address private constant IDLE_V4_REFERRAL_ACCOUNT =
        ...
        returns (address[] memory rewardsTokens_)
    {
        ...
        rewardsTokens_ = new address[](
            idleTokenContract.getGovTokensAmounts(address(0)).length
        );
    ...
    function __idleV4Lend(
        ...
        IIdleTokenV4(_idleToken).mintIdleToken(
            _underlyingAmount,
            true,
            IDLE_V4_REFERRAL_ACCOUNT
        );
    }

    /// @dev Helper to redeem IdleToken for underlying
    function __idleV4Redeem(address _idleToken, uint256 _idleTokenAmount)
        internal
    {

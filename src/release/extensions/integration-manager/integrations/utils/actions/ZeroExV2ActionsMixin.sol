...
abstract contract ZeroExV2ActionsMixin is AssetHelpers, MathHelpers {
    ...
    function __zeroExV2TakeOrder(
        ...
        if (_order.takerFee > 0) {
            bytes memory zrxData = IZeroExV2(ZERO_EX_V2_EXCHANGE)
                .ZRX_ASSET_DATA();
            __approveAssetMaxAsNeeded(
        ...
        // Execute order
        IZeroExV2(ZERO_EX_V2_EXCHANGE).fillOrder(
            _order,
            _takerAssetFillAmount,
            _signature
        );
        ...
        returns (address assetProxy_)
    {
        ...
        assetProxy_ = IZeroExV2(getZeroExV2Exchange()).getAssetProxy(
            assetProxyId
        );
    }
    ...
    function getZeroExV2Exchange()
        public
        view
        returns (address zeroExV2Exchange_)
    {

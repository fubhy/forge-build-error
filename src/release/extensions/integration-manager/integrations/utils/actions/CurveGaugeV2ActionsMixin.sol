...
abstract contract CurveGaugeV2ActionsMixin is AssetHelpers {
    ...
    function __curveGaugeV2ClaimRewards(address _gauge, address _target)
        internal
    {
    ...
    {
        address[] memory lpRewardsTokensWithEmpties = new address[](
            CURVE_GAUGE_V2_MAX_REWARDS
        );
        uint256 rewardsTokensCount;
        for (uint256 i; i < CURVE_GAUGE_V2_MAX_REWARDS; i++) {
            address rewardToken = ICurveLiquidityGaugeV2(_gauge).reward_tokens(
                i
            );

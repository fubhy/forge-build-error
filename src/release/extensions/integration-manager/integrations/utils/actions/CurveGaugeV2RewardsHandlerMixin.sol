...
abstract contract CurveGaugeV2RewardsHandlerMixin is CurveGaugeV2ActionsMixin {
    ...
    function __curveGaugeV2ClaimAllRewards(address _gauge, address _target)
        internal
    {
        if (__curveGaugeV2MinterExists()) {
            // Claim owed $CRV via Minter (only on Ethereum mainnet)
            ICurveMinter(CURVE_GAUGE_V2_REWARDS_HANDLER_MINTER).mint_for(
                _gauge,
                _target
            );
        }
    ...
    function getCurveGaugeV2RewardsHandlerCrvToken()
        public
        view
        returns (address crvToken_)
    {
    ...
    function getCurveGaugeV2RewardsHandlerMinter()
        public
        view
        returns (address minter_)
    {

...
abstract contract CurveLiquidityActionsMixin is AssetHelpers {
  ...
  bytes4
    private constant CURVE_REMOVE_LIQUIDITY_ONE_COIN_USE_UNDERLYINGS_SELECTOR =
    0x517a55a3;
  ...
  function __curveAddLiquidity(
    ...
    for (uint256 i; i < _squashedOutgoingAssets.length; i++) {
      if (_squashedOutgoingAssets[i] == getCurveLiquidityWrappedNativeAsset()) {
        ...
        outgoingNativeAssetAmount = ERC20(getCurveLiquidityWrappedNativeAsset())
          .balanceOf(address(this));
        IWETH(getCurveLiquidityWrappedNativeAsset()).withdraw(
          outgoingNativeAssetAmount
        );
      } else {
    ...
    (bool success, bytes memory returnData) = _pool.call{
      value: outgoingNativeAssetAmount
    }(
  ...
  function __curveAddLiquidityEncodeSelector(
  ...
  function __curveRemoveLiquidityEncodeSelector(
    uint256 _numberOfCoins,
    bool _useUnderlyings
  ) private pure returns (bytes4 selector_) {
    string memory finalArgOrEmpty;
  ...
  function getCurveLiquidityWrappedNativeAsset()
    public
    view
    returns (address addressProvider_)
  {

...
abstract contract CurveExchangeActionsMixin is AssetHelpers {
    ...
    function __curveTakeOrder(
        ...
        address swaps = ICurveAddressProvider(CURVE_EXCHANGE_ADDRESS_PROVIDER)
            .get_address(2);
        ...
        if (_outgoingAsset == CURVE_EXCHANGE_WETH_TOKEN) {
            ...
            IWETH(payable(CURVE_EXCHANGE_WETH_TOKEN)).deposit{
                value: receivedAmount
            }();
            ERC20(CURVE_EXCHANGE_WETH_TOKEN).safeTransfer(
                _recipient,
                receivedAmount
            );
        } else {
    ...
    function getCurveExchangeWethToken()
        public
        view
        returns (address curveExchangeWethToken_)
    {

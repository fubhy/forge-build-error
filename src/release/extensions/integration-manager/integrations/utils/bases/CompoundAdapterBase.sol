...
abstract contract CompoundAdapterBase is AdapterBase, CompoundActionsMixin {
        ...
        )
    {
        (
            address cToken,
            uint256 tokenAmount,
            uint256 minCTokenAmount
        ) = __decodeCallArgs(_actionData);
        address token = CompoundPriceFeed(COMPOUND_PRICE_FEED)
            .getTokenFromCToken(cToken);
        require(
            token != address(0),
            "__parseAssetsForLend: Unsupported cToken"
        );
        ...
        )
    {
        ...
        require(
            token != address(0),
            "__parseAssetsForRedeem: Unsupported cToken"
        );

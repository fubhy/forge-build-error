...
contract ZeroExV2Adapter is
    AdapterBase,
    FundDeployerOwnerMixin,
    ZeroExV2ActionsMixin
{
        ...
        postActionIncomingAssetsTransferHandler(_vaultProxy, _assetData)
    {
        ...
        (, , , bytes memory signature) = __decodeZeroExOrderArgs(
            encodedZeroExOrderArgs
        );
    ...
    {
        require(
            _selector == TAKE_ORDER_SELECTOR,
            "parseAssetsForAction: _selector invalid"
        );
        ...
        if (order.takerFee > 0) {
            ...
            if (takerFeeAsset == makerAsset) {
                ...
                minIncomingAssetAmounts_[0] = minIncomingAssetAmounts_[0].sub(
                    takerFeeFillAmount
                );
    ...
    function addAllowedMakers(address[] calldata _accountsToAdd)
        external
    ...
    {
        require(
            _accountsToRemove.length > 0,
            "removeAllowedMakers: Empty _accountsToRemove"
        );
    ...
    function __addAllowedMakers(address[] memory _accountsToAdd) private {
        require(
            _accountsToAdd.length > 0,
            "__addAllowedMakers: Empty _accountsToAdd"
        );

        for (uint256 i; i < _accountsToAdd.length; i++) {
            require(
                !isAllowedMaker(_accountsToAdd[i]),
                "__addAllowedMakers: Value already set"
            );
    ...
    function isAllowedMaker(address _who)
        public
        view
        returns (bool isAllowedMaker_)
    {

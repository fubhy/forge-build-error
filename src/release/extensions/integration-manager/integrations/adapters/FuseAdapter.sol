...
contract FuseAdapter is CompoundAdapterBase {
    constructor(
        ...
        public
        CompoundAdapterBase(_integrationManager, _fusePriceFeed, _wethToken)
    {}
    ...
    function claimRewards(
        ...
        (address[] memory cTokens, address poolComptroller) = __decodeClaimArgs(
            _actionData
        );
        ...
        for (uint256 i; i < rewardsDistributors.length; i++) {
            IFuseRewardsDistributor(rewardsDistributors[i]).claimRewards(
                _vaultProxy,
                cTokens
            );
        }

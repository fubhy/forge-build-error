  ...
  MathHelpers
{
  ...
  function __actionCallOnAccountingModule(bytes memory _actionArgs) private {
    IArbitraryLoanAccountingModule(getAccountingModule()).receiveCallFromLoan(
      _actionArgs
    );
  }
  ...
  function __actionConfigureLoan(bytes memory _actionArgs) private {
    require(
      getLoanAsset() == address(0),
      "__actionConfigureLoan: Already configured"
    );
    ...
    require(
      loanAssetMem != address(0),
      "__actionConfigureLoan: Empty loan asset"
    );
  ...
  function __actionUpdateBorrowableAmount(bytes memory _actionArgs)
    private
    onlyNotClosed
  {
  ...
  function __reconcile(address[] memory _extraAssetsToSweep) private {
    ...
    uint256 repayableAmount = __subOrZero(
      loanAssetBalance,
      getBorrowableAmount()
    );
    ...
    returns (address[] memory assets_, uint256[] memory amounts_)
  {
    ...
    if (accountingModuleMem != address(0)) {
      totalBalance = IArbitraryLoanAccountingModule(accountingModuleMem)
        .calcFaceValue(getTotalBorrowed(), getTotalRepaid());
    } else {
  ...
  function getAccountingModule()
    public
    view
    returns (address accountingModule_)
  {
  ...
  function getBorrowableAmount()
    public
    view
    returns (uint256 borrowableAmount_)
  {

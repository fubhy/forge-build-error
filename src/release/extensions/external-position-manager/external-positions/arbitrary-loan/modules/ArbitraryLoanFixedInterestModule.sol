  ...
  MathHelpers
{
  ...
  event TotalPrincipalRepaidUpdatedForLoan(
    address indexed loan,
    uint256 totalPrincipalRepaid
  );

  event TotalInterestUpdatedForLoan(
    address indexed loan,
  ...
  function configure(bytes memory _configData) external override {
    ...
    (
      ...
      bool faceValueIsPrincipalOnly
        ...
        _configData,
        (uint96, uint96, uint32, RepaymentTrackingType, bool)
      );
    ...
    require(
      !faceValueIsPrincipalOnly ||
        repaymentTrackingType != RepaymentTrackingType.None,
  ...
  function __checkpointInterest(
    ...
    uint256 uncachedInterest = __calcUncachedInterest(
      _loan,
      _totalBorrowed,
      _totalRepaid
    );
  ...
  function __reconcilePrincipalRepaid(
    ...
    uint256 principalOutstanding = _totalBorrowed.sub(
      accountingInfo.totalPrincipalRepaid
    );
    ...
    if (
      accountingInfo.repaymentTrackingType ==
      RepaymentTrackingType.PrincipalFirst
    ) {
      ...
      if (_repayAmount >= prevLoanBalance) {
        ...
        if (_repayAmount > interestRemaining) {
          nextTotalPrincipalRepaid = uint256(
            accountingInfo.totalPrincipalRepaid
          ).add(_repayAmount).sub(interestRemaining);
        }
      }
    }
  ...
  function __safeCastUint112(uint256 value)
    private
    pure
    returns (uint112 castedValue_)
  {
  ...
  function __calcUncachedInterest(
    ...
    if (
      accountingInfo.totalInterestCachedTimestamp >= accountingInfo.maturity
    ) {
      return
        __calcContinuouslyCompoundedInterest(
          loanBalanceAtCheckpoint,
    ...
    uint256 preMaturityInterest = __calcContinuouslyCompoundedInterest(
      loanBalanceAtCheckpoint,
      accountingInfo.scaledPerSecondRatePreMaturity,
      uint256(accountingInfo.maturity).sub(
        accountingInfo.totalInterestCachedTimestamp
      )
    );

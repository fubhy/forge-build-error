...
contract ArbitraryLoanTotalNominalDeltaOracleModule is
  IArbitraryLoanAccountingModule
{
  ...
  function configure(bytes memory _configData) external override {
    ...
    (address oracle, uint32 stalenessThreshold) = abi.decode(
      _configData,
      (address, uint32)
    );
  ...
  function __calcLoanBalance(
    ...
    int256 totalBalanceInt = int256(_totalBorrowed).add(oracleValue).sub(
      int256(_totalRepaid)
    );

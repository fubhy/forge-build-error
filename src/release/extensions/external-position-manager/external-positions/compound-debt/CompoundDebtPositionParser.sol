...
contract CompoundDebtPositionParser is IExternalPositionParser {
        ...
        )
    {
        ...
        if (
            _actionId ==
            uint256(ICompoundDebtPosition.ExternalPositionActions.AddCollateral)
        ) {
            assetsToTransfer_ = assets;
            amountsToTransfer_ = amounts;
        } else if (
            _actionId ==
            uint256(ICompoundDebtPosition.ExternalPositionActions.RepayBorrow)
        ) {
            ...
            for (uint256 i; i < assets.length; i++) {
                ...
                if (amounts[i] == type(uint256).max) {
                    ...
                    amounts[i] = ICERC20(cTokens[i]).borrowBalanceStored(
                        _externalPosition
                    );
                }
            }
            ...
            _actionId ==
            uint256(ICompoundDebtPosition.ExternalPositionActions.Borrow) ||
            _actionId ==
            uint256(
                ICompoundDebtPosition.ExternalPositionActions.RemoveCollateral
            )
        ) {
            assetsToReceive_ = assets;
        } else if (
            _actionId ==
            uint256(ICompoundDebtPosition.ExternalPositionActions.ClaimComp)
        ) {
    ...
    {
        (assets_, amounts_, data_) = abi.decode(
            _encodeActionArgs,
    ...
    function __validateActionData(
        ...
        if (
            ...
            for (uint256 i; i < _assets.length; i++) {
                require(
                    IValueInterpreter(getValueInterpreter()).isSupportedAsset(
                        _assets[i]
                    ),
    ...
    function __validateCTokens(
        ...
        for (uint256 i; i < _cTokens.length; i++) {
            require(
                CompoundPriceFeed(getCompoundPriceFeed()).getTokenFromCToken(
                    _cTokens[i]
                ) == _tokens[i],
    ...
    function getCompoundPriceFeed()
        public
        view
        returns (address compoundPriceFeed_)
    {

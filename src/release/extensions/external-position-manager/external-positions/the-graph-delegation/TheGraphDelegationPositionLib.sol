  ...
  TheGraphDelegationPositionDataDecoder
{
  ...
  function init(bytes memory) external override {
    ...
    GRT_TOKEN_CONTRACT.safeApprove(
      address(GRAPH_STAKING_CONTRACT),
      type(uint256).max
    );
  }
  ...
  function __undelegate(bytes memory _actionArgs) private {
    (address indexer, uint256 shares) = __decodeUndelegateActionArgs(
      _actionArgs
    );
  ...
  function __withdraw(bytes memory _actionArgs) private {
    (address indexer, address nextIndexer) = __decodeWithdrawActionArgs(
      _actionArgs
    );
    GRAPH_STAKING_CONTRACT.withdrawDelegated(indexer, nextIndexer);
    (uint256 delegationShares, uint256 tokensLocked, ) = GRAPH_STAKING_CONTRACT
      .getDelegation(indexer, address(this));
  ...
  function getDelegationGrtValue(address _indexer)
    public
    view
    returns (uint256 grtValue_)
  {
    (uint256 delegationShares, uint256 tokensLocked, ) = GRAPH_STAKING_CONTRACT
      .getDelegation(_indexer, address(this));

    (, , , , uint256 poolTokens, uint256 poolShares) = GRAPH_STAKING_CONTRACT
      .delegationPools(_indexer);
  ...
  function isDelegatorTo(address _indexer)
    public
    view
    returns (bool isDelegator_)
  {

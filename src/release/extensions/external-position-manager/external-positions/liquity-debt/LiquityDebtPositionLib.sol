...
contract LiquityDebtPositionLib is
  ILiquityDebtPosition,
  LiquityDebtPositionDataDecoder
{
  ...
  function __addCollateral(
    ...
    ILiquityBorrowerOperations(LIQUITY_BORROWER_OPERATIONS).addColl{
      value: _amount
  ...
  function __closeTrove() private {
    ...
    ERC20(LUSD_TOKEN).safeTransfer(
      msg.sender,
      ERC20(LUSD_TOKEN).balanceOf(address(this))
    );
    ...
    returns (address[] memory assets_, uint256[] memory amounts_)
  {
    ...
    amounts_[0] = ILiquityTroveManager(LIQUITY_TROVE_MANAGER).getTroveDebt(
      address(this)
    );
  ...
  {
    amounts_ = new uint256[](1);
    amounts_[0] = ILiquityTroveManager(LIQUITY_TROVE_MANAGER).getTroveColl(
      address(this)
    );

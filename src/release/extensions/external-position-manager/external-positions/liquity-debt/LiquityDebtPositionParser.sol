...
contract LiquityDebtPositionParser is
    IExternalPositionParser,
    LiquityDebtPositionDataDecoder
{
        ...
        )
    {
        if (_actionId == uint256(ILiquityDebtPosition.Actions.OpenTrove)) {
            (, uint256 collateralAmount, , , ) = __decodeOpenTroveArgs(
                _encodedActionArgs
            );
        ...
        if (_actionId == uint256(ILiquityDebtPosition.Actions.AddCollateral)) {
            (uint256 collateralAmount, , ) = __decodeAddCollateralActionArgs(
                _encodedActionArgs
            );
        ...
        if (
            _actionId == uint256(ILiquityDebtPosition.Actions.RemoveCollateral)
        ) {
            assetsToReceive_ = new address[](1);
            assetsToReceive_[0] = WETH_TOKEN;
        } else if (
            _actionId == uint256(ILiquityDebtPosition.Actions.RepayBorrow)
        ) {
            (uint256 lusdAmount, , ) = __decodeRepayBorrowActionArgs(
                _encodedActionArgs
            );
            assetsToTransfer_ = new address[](1);
            amountsToTransfer_ = new uint256[](1);
            ...
            _actionId == uint256(ILiquityDebtPosition.Actions.CloseTrove)
        ) {
            uint256 lusdAmount = ILiquityTroveManager(LIQUITY_TROVE_MANAGER)
                .getTroveDebt(_externalPosition);

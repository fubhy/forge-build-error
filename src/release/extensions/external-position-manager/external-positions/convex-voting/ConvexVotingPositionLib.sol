  ...
  AssetHelpers
{
  ...
  IConvexVlCvxExtraRewardDistribution
    private immutable VLCVX_EXTRA_REWARDS_CONTRACT;
  ...
  constructor(
    ...
    SNAPSHOT_DELEGATE_REGISTRY = ISnapshotDelegateRegistry(
      _snapshotDelegateRegistry
    );
    VLCVX_CONTRACT = IConvexCvxLockerV2(_vlCvx);
    VLCVX_EXTRA_REWARDS_CONTRACT = IConvexVlCvxExtraRewardDistribution(
      _vlCvxExtraRewards
    );
    VOTIUM_MULTI_MERKLE_STASH_CONTRACT = IVotiumMultiMerkleStash(
  ...
  function __claimRewards(bytes memory _actionArgs) private {
    ...
    for (uint256 i; i < extraRewardTokens.length; i++) {
      VLCVX_EXTRA_REWARDS_CONTRACT.getReward(
        address(this),
        extraRewardTokens[i]
      );
    }
    ...
    if (votiumClaims.length > 0) {
      VOTIUM_MULTI_MERKLE_STASH_CONTRACT.claimMulti(
        address(this),
        votiumClaims
      );
    }

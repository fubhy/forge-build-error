...
contract AaveDebtPositionParser is
    ...
    AaveDebtPositionDataDecoder
{
        ...
        )
    {
        if (_actionId == uint256(IAaveDebtPosition.Actions.AddCollateral)) {
            ...
            _actionId == uint256(IAaveDebtPosition.Actions.RemoveCollateral)
        ) {
            // Lib validates that each is a valid collateral asset
            (assetsToReceive_, ) = __decodeRemoveCollateralActionArgs(
                _encodedActionArgs
            );
        } else if (
            _actionId == uint256(IAaveDebtPosition.Actions.RepayBorrow)
        ) {
            // Lib validates that each is a valid borrowed asset
            (
                assetsToTransfer_,
                amountsToTransfer_
            ) = __decodeRepayBorrowActionArgs(_encodedActionArgs);
            ...
            for (uint256 i; i < assetsToTransfer_.length; i++) {
                if (amountsToTransfer_[i] == type(uint256).max) {
                    ...
                    amountsToTransfer_[i] = ERC20(debtToken).balanceOf(
                        _externalPosition
                    );
                }
            }
    ...
    function __validateSupportedAssets(address[] memory _assets) private view {
        for (uint256 i; i < _assets.length; i++) {
            require(
                IValueInterpreter(VALUE_INTERPRETER).isSupportedAsset(
                    _assets[i]
                ),

  ...
  AssetHelpers
{
  ...
  function __repayBorrowedAssets(bytes memory actionArgs) private {
    ...
    for (uint256 i; i < tokens.length; i++) {
      require(
        assetIsBorrowed(tokens[i]),
        "__repayBorrowedAssets: Invalid borrowed asset"
      );
      ...
      if (
        ERC20(getDebtTokenForBorrowedAsset(tokens[i])).balanceOf(
          address(this)
        ) == 0
      ) {
    ...
    returns (address[] memory assets_, uint256[] memory amounts_)
  {
    ...
    for (uint256 i; i < assets_.length; i++) {
      amounts_[i] = ERC20(getDebtTokenForBorrowedAsset(assets_[i])).balanceOf(
        address(this)
      );
    }
  ...
  function assetIsBorrowed(address _asset)
    public
    view
    returns (bool isBorrowed_)
  {

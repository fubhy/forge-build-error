  ...
  AssetHelpers
{
  ...
  constructor(
    address _convertibleMarketcontract,
    address _initialConvertibleOfferingMarket
  ) public {
    CONVERTIBLE_MARKET_CONTRACT = ISolvV2ConvertibleMarket(
      _convertibleMarketcontract
    );
  ...
  function __actionBuyOffering(bytes memory _actionArgs) private {
    (
      ...
      uint24 offerId,
      uint128 units
    ) = __decodeBuyOfferingActionArgs(_actionArgs);
  ...
  function __actionBuySaleByUnits(bytes memory _actionArgs) private {
    ...
    ERC20(sale.currency).safeApprove(
  ...
  function __actionClaim(bytes memory _actionArgs) private {
    (address voucher, uint32 tokenId, uint256 units) = __decodeClaimActionArgs(
      _actionArgs
    );
  ...
  function __addVoucherTokenId(address _voucher, uint32 _tokenId) private {
    voucherTokenIds.push(
      VoucherTokenId({ tokenId: _tokenId, voucher: _voucher })
    );
  ...
  function __removeVoucherTokenId(address _voucher, uint32 _tokenId) private {
    ...
    for (uint256 i; i < voucherTokenIdsLength; i++) {
      ...
      if (
        voucherTokenId.tokenId == _tokenId && voucherTokenId.voucher == _voucher
      ) {
  ...
  function getVoucherTokenIds()
    public
    view
    returns (VoucherTokenId[] memory voucherTokenIds_)
  {
  ...
  function onVNFTReceived(
    ...
    return
      bytes4(
        keccak256("onVNFTReceived(address,address,uint256,uint256,bytes)")
      );
  }
}

  ...
  SolvV2ConvertibleBuyerPositionDataDecoder
{
  ...
  constructor(
    address _convertibleMarket,
    address _initialConvertibleOfferingMarket
  ) public {
  ...
  {
    if (
      _actionId == uint256(ISolvV2ConvertibleBuyerPosition.Actions.BuyOffering)
    ) {
      (
        address voucher,
        uint24 offerId,
        uint128 units
      ) = __decodeBuyOfferingActionArgs(_encodedActionArgs);

      ISolvV2InitialConvertibleOfferingMarket.Offering
        memory offering = INITIAL_CONVERTIBLE_OFFERING_MARKET_CONTRACT
          .offerings(offerId);

      assetsToTransfer_ = new address[](1);
      assetsToTransfer_[0] = offering.currency;

      uint256 voucherPrice = INITIAL_CONVERTIBLE_OFFERING_MARKET_CONTRACT
        .getPrice(offerId);
      ...
      uint256 amount = uint256(units).mul(voucherPrice).div(
        10**uint256(market.decimals)
      );
      ...
      _actionId ==
      uint256(ISolvV2ConvertibleBuyerPosition.Actions.BuySaleByAmount)
    ) {
      (uint24 saleId, uint256 amount) = __decodeBuySaleByAmountActionArgs(
        _encodedActionArgs
      );

      ISolvV2ConvertibleMarket.Sale memory sale = CONVERTIBLE_MARKET_CONTRACT
        .sales(saleId);
      assetsToTransfer_ = new address[](1);
      ...
      _actionId ==
      uint256(ISolvV2ConvertibleBuyerPosition.Actions.BuySaleByUnits)
    ) {
      (uint24 saleId, uint128 units) = __decodeBuySaleByUnitsActionArgs(
        _encodedActionArgs
      );
      ...
      ISolvV2ConvertibleMarket.Market
        memory market = CONVERTIBLE_MARKET_CONTRACT.markets(sale.voucher);
      uint128 price = CONVERTIBLE_MARKET_CONTRACT.getPrice(saleId);
      uint256 amount = uint256(units).mul(uint256(price)).div(
        uint256(market.precision)
      );
      ...
      _actionId == uint256(ISolvV2ConvertibleBuyerPosition.Actions.Claim)
    ) {
      (address voucher, uint256 tokenId, ) = __decodeClaimActionArgs(
        _encodedActionArgs
      );
      ...
      _actionId == uint256(ISolvV2ConvertibleBuyerPosition.Actions.Reconcile)
    ) {
      ISolvV2ConvertibleBuyerPosition externalPositionContract = ISolvV2ConvertibleBuyerPosition(
          _externalPosition
        );

      SolvV2ConvertibleBuyerPositionLib.Sale[]
        memory sales = externalPositionContract.getSales();
      ...
      _actionId == uint256(ISolvV2ConvertibleBuyerPosition.Actions.RemoveSale)
    ) {

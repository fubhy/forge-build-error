    ...
    IExternalPositionParser
{
    ...
    constructor(address _maplePoolFactory, address _mapleMplRewardsFactory)
        public
    {
        ...
        )
    {
        ...
        if (_actionId == uint256(IMapleLiquidityPosition.Actions.Lend)) {
            ...
            _actionId == uint256(IMapleLiquidityPosition.Actions.LendAndStake)
        ) {
            (
                address pool,
                ,
            ...
            _actionId == uint256(IMapleLiquidityPosition.Actions.Redeem)
        ) {
            ...
            _actionId ==
            uint256(IMapleLiquidityPosition.Actions.UnstakeAndRedeem)
        ) {
            (address pool, , ) = __decodeUnstakeAndRedeemActionArgs(
                _encodedActionArgs
            );
            ...
            _actionId == uint256(IMapleLiquidityPosition.Actions.ClaimInterest)
        ) {
    ...
    function __validateActionData(uint256 _actionId, bytes memory _actionArgs)
        private
        view
    {
        if (_actionId == uint256(IMapleLiquidityPosition.Actions.Lend)) {
            ...
            _actionId == uint256(IMapleLiquidityPosition.Actions.IntendToRedeem)
        ) {
            ...
            _actionId == uint256(IMapleLiquidityPosition.Actions.Stake)
        ) {
            (address rewardsContract, address pool, ) = __decodeStakeActionArgs(
                _actionArgs
            );
            ...
            _actionId == uint256(IMapleLiquidityPosition.Actions.Unstake)
        ) {
            (address rewardsContract, ) = __decodeUnstakeActionArgs(
                _actionArgs
            );
            ...
            ) = __decodeUnstakeAndRedeemActionArgs(_actionArgs);
            ...
            _actionId == uint256(IMapleLiquidityPosition.Actions.ClaimRewards)
        ) {
            address rewardsContract = __decodeClaimRewardsActionArgs(
                _actionArgs
            );
    ...
    function __validateRewardsContract(address _rewardsContract) private view {
        require(
            IMapleMplRewardsFactory(MAPLE_MPL_REWARDS_FACTORY).isMplRewards(
                _rewardsContract
            ),

  ...
  AssetHelpers
{
  ...
  function __calcLiquidityAssetValueOfPoolTokens(
    ...
    uint256 liquidityAssetDecimalsFactor = 10 **
      (uint256(ERC20(_liquidityAsset).decimals()));
  ...
  function __lendAndStakeAction(bytes memory _actionArgs) private {
    ...
    __stake(
      rewardsContract,
      pool,
      poolTokenBalanceAfter.sub(poolTokenBalanceBefore)
    );
  }
  ...
  function __redeemAction(bytes memory actionArgs) private {
    (address pool, uint256 liquidityAssetAmount) = __decodeRedeemActionArgs(
      actionArgs
    );
  ...
  function __stake(
    ...
    IMaplePool(_pool).increaseCustodyAllowance(
      _rewardsContract,
      _poolTokenAmount
    );
  ...
  function __stakeAction(bytes memory _actionArgs) private {
    ...
    ) = __decodeStakeActionArgs(_actionArgs);

    __stake(rewardsContract, pool, poolTokenAmount);
  }

  /// @dev Unstakes assets from a rewardsContract
  function __unstake(address _rewardsContract, uint256 _poolTokenAmount)
    private
  {
  ...
  function __unstakeAction(bytes memory _actionArgs) private {
    (
      address rewardsContract,
      uint256 poolTokenAmount
    ) = __decodeUnstakeActionArgs(_actionArgs);
  ...
  function __unstakeAndRedeemAction(bytes memory actionArgs) private {
    ...
    __redeem(
      pool,
      __calcLiquidityAssetValueOfPoolTokens(liquidityAsset, poolTokenAmount)
    );
    ...
    returns (address[] memory assets_, uint256[] memory amounts_)
  {
    ...
    for (uint256 i; i < usedLendingPoolsLength; i++) {
      ...
      uint256 accumulatedInterest = poolContract.withdrawableFundsOf(
        address(this)
      );
      uint256 accumulatedLosses = poolContract.recognizableLossesOf(
        address(this)
      );

      amounts_[i] = liquidityAssetBalance.add(accumulatedInterest).sub(
        accumulatedLosses
      );
    }

    ...
    PermissionedVaultActionMixin
{
    ...
    function receiveCallFromComptroller(
        ...
        require(
            ...
            "receiveCallFromComptroller: Fund is not valid"
        );
        ...
        if (
            _actionId ==
            uint256(ExternalPositionManagerActions.CreateExternalPosition)
        ) {
            ...
            _actionId ==
            uint256(ExternalPositionManagerActions.CallOnExternalPosition)
        ) {
            (
                address externalPosition,
                uint256 actionId,
            ...
            _actionId ==
            uint256(ExternalPositionManagerActions.RemoveExternalPosition)
        ) {
            __executeRemoveExternalPosition(
                _caller,
                comptrollerProxy,
                _callArgs
            );
            ...
            _actionId ==
            uint256(ExternalPositionManagerActions.ReactivateExternalPosition)
        ) {
            __reactivateExternalPosition(
                _caller,
                comptrollerProxy,
                vaultProxy,
                _callArgs
    ...
    function __createExternalPosition(
        ...
        require(
            parser != address(0),
            "__createExternalPosition: Invalid typeId"
        );
        ...
        address externalPosition = ExternalPositionFactory(
        ...
        if (callOnExternalPositionCallArgs.length != 0) {
            ...
            (
                ...
                uint256 actionId,
                bytes memory actionArgs
            ) = __decodeCallOnExternalPositionCallArgs(
    ...
    function __reactivateExternalPosition(
        ...
        require(
            ExternalPositionFactory(getExternalPositionFactory())
                .isExternalPositionProxy(externalPosition),
            "__reactivateExternalPosition: Account provided is not a valid external position"
        );

        require(
            IExternalPositionProxy(externalPosition).getVaultProxy() ==
                _vaultProxy,
    ...
    function updateExternalPositionTypesInfo(
        ...
        require(
            _typeIds.length == _parsers.length &&
                _libs.length == _parsers.length,
        ...
        for (uint256 i; i < _typeIds.length; i++) {
            ...
            emit ExternalPositionTypeInfoUpdated(
                _typeIds[i],
                _libs[i],
                _parsers[i]
            );
    ...
    function getExternalPositionFactory()
        public
        view
        returns (address externalPositionFactory_)
    {

...
contract MinAssetBalancesPostRedemptionPolicy is PolicyBase {
  ...
  mapping(address => mapping(address => uint256))
    private comptrollerProxyToAssetToMinBalance;
  ...
  function addFundSettings(
    ...
    require(
      assets.length == minBalances.length,
      "addFundSettings: Unequal array lengths"
    );

    for (uint256 i; i < assets.length; i++) {
      comptrollerProxyToAssetToMinBalance[_comptrollerProxy][
        assets[i]
      ] = minBalances[i];

      emit MinAssetBalanceAddedForFund(
        _comptrollerProxy,
        assets[i],
        minBalances[i]
      );
    ...
    returns (IPolicyManager.PolicyHook[] memory implementedHooks_)
  {
    ...
    implementedHooks_[0] = IPolicyManager
      .PolicyHook
  ...
  function validateRule(
    ...
    ) = __decodeRedeemSharesForSpecificAssetsValidationData(_encodedArgs);

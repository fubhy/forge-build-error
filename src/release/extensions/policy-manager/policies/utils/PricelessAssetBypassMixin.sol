...
abstract contract PricelessAssetBypassMixin {
  ...
  event PricelessAssetBypassed(
    address indexed comptrollerProxy,
    address indexed asset
  );

  event PricelessAssetTimelockStarted(
    address indexed comptrollerProxy,
    address indexed asset
  );
  ...
  function startAssetBypassTimelock(address _asset) external {
    ...
    try
      ValueInterpreter(getPricelessAssetBypassValueInterpreter())
        ...
        )
    {
      ...
      comptrollerProxyToAssetToBypassWindowStart[comptrollerProxy][
        _asset
  ...
  {
    uint256 windowStart = getAssetBypassWindowStartForFund(
      _comptrollerProxy,
      _asset
    );
  ...
  function __calcValueExcludingBypassablePricelessAsset(
    ...
    try
      ValueInterpreter(getPricelessAssetBypassValueInterpreter())
        .calcCanonicalAssetValue(_baseAsset, _baseAssetAmount, _quoteAsset)
  ...
  function getAssetBypassWindowStartForFund(
    address _comptrollerProxy,
    address _asset
  ) public view returns (uint256 windowStart_) {
    return
      comptrollerProxyToAssetToBypassWindowStart[_comptrollerProxy][_asset];
  }
  ...
  function getPricelessAssetBypassTimeLimit()
    public
    view
    returns (uint256 timeLimit_)
  {
  ...
  function getPricelessAssetBypassTimelock()
    public
    view
    returns (uint256 timelock_)
  {
  ...
  function getPricelessAssetBypassWethToken()
    public
    view
    returns (address wethToken_)
  {

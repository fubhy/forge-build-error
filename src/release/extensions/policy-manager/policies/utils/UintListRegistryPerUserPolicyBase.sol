...
abstract contract UintListRegistryPerUserPolicyBase is PolicyBase {
    ...
    constructor(address _policyUser, address _uintListRegistry)
        public
    ...
    function __createUintListFromData(
        address _vaultProxy,
        ...
        return
            UINT_LIST_REGISTRY_CONTRACT.createList(
                _vaultProxy,
                updateType,
                initialItems
            );
    ...
    function __decodeNewListData(bytes memory _newListData)
        ...
        pure
        returns (
            UintListRegistry.UpdateType updateType_,
            uint256[] memory initialItems_
        )
    {
        return
            abi.decode(_newListData, (UintListRegistry.UpdateType, uint256[]));
    }

...
abstract contract AddressListRegistryPerUserPolicyBase is PolicyBase {
    ...
    mapping(address => mapping(address => uint256[]))
        private comptrollerProxyToUserToListIds;

    constructor(address _policyUser, address _addressListRegistry)
        public
        PolicyBase(_policyUser)
    {
        ADDRESS_LIST_REGISTRY_CONTRACT = AddressListRegistry(
    ...
    function addFundSettings(
        address _comptrollerProxy,
        bytes calldata _encodedSettings
    ) external virtual override onlyPolicyManager {
    ...
    function __updateListsForFund(
        address _comptrollerProxy,
        bytes calldata _encodedSettings
    ) internal {
        (
            address[] memory users,
            bytes[] memory listsData
        ) = __decodePolicySettings(_encodedSettings);

        require(
            users.length == listsData.length,
            "__updateListsForFund: unequal arrays"
        );

        for (uint256 i; i < listsData.length; i++) {
            __updateListsForFundAndUser(
                _comptrollerProxy,
                users[i],
                listsData[i]
            );
    ...
    function __createAddressListFromData(
        address _vaultProxy,
        bytes memory _newListData
    ) private returns (uint256 listId_) {
        (
        ...
        return
            ADDRESS_LIST_REGISTRY_CONTRACT.createList(
                _vaultProxy,
                updateType,
                initialItems
            );
    ...
    function __decodeNewListData(bytes memory _newListData)
        ...
        pure
        returns (
            AddressListRegistry.UpdateType updateType_,
            address[] memory initialItems_
        )
    {
        return
            abi.decode(
                _newListData,
                (AddressListRegistry.UpdateType, address[])
            );
    }
    ...
    function __decodeUserListsData(bytes memory _listData)
        ...
        pure
        returns (
            uint256[] memory existingListIds_,
            bytes[] memory newListsData_
        )
    {
    ...
    function __updateListsForFundAndUser(
        ...
        (
            ...
            bytes[] memory newListsData
        ) = __decodeUserListsData(_listData);

        // Clear the previously stored list ids as needed
        if (
            comptrollerProxyToUserToListIds[_comptrollerProxy][_user].length > 0
        ) {
            delete comptrollerProxyToUserToListIds[_comptrollerProxy][_user];
        }

        uint256[] memory nextListIds = new uint256[](
            existingListIds.length + newListsData.length
        );
        ...
        if (nextListIds.length > 0) {
            ...
            if (newListsData.length > 0) {
                address vaultProxy = ComptrollerLib(_comptrollerProxy)
                    .getVaultProxy();
                for (uint256 i; i < newListsData.length; i++) {
                    ...
                    comptrollerProxyToUserToListIds[_comptrollerProxy][_user]
                        .push(nextListIds[nextListIdsIndex]);
                }
            }
        }

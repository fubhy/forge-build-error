...
abstract contract AddressListRegistryPolicyBase is PolicyBase {
    ...
    ) internal returns (uint256 listId_) {
        (
    ...
    function __updateListsForFund(
        ...
        (uint256[] memory existingListIds, bytes[] memory newListsData) = abi
            .decode(_encodedSettings, (uint256[], bytes[]));
        ...
        require(
            ...
            "__updateListsForFund: No lists specified"
        );
        ...
        for (uint256 i; i < existingListIds.length; i++) {
            ...
            comptrollerProxyToListIds[_comptrollerProxy].push(
                existingListIds[i]
            );
        }
    ...
    function getAddressListRegistry()
        public
        view
        returns (address addressListRegistry_)
    {

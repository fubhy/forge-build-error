  ...
  PricelessAssetBypassMixin
{
  ...
  function addFundSettings(
    ...
    require(
      tolerance < ONE_HUNDRED_PERCENT,
      "addFundSettings: Max tolerance exceeded"
    );
  ...
  function validateRule(
    ...
    uint256 tolerance = comptrollerProxyToPolicyInfo[_comptrollerProxy]
      .tolerance;

    return
      __updateCumulativeSlippage(_comptrollerProxy, newSlippage, tolerance) <=
      tolerance;
  }
  ...
  function __isBypassableAction(address _adapter)
    private
    view
  ...
  function __updateCumulativeSlippage(
    ...
    PolicyInfo storage policyInfo = comptrollerProxyToPolicyInfo[
      _comptrollerProxy
    ];
    ...
    if (nextCumulativeSlippage_ > 0) {
      ...
      if (cumulativeSlippageToRestore < nextCumulativeSlippage_) {
        nextCumulativeSlippage_ = nextCumulativeSlippage_.sub(
          cumulativeSlippageToRestore
        );
      } else {
    ...
    emit CumulativeSlippageUpdatedForFund(
      _comptrollerProxy,
      nextCumulativeSlippage_
    );
  ...
  function getTolerancePeriodDuration()
    public
    view
    returns (uint256 tolerancePeriodDuration_)
  {

...
contract AllowedExternalPositionTypesPerManagerPolicy is
    UintListRegistryPerUserPolicyBase
{
        ...
        returns (IPolicyManager.PolicyHook[] memory implementedHooks_)
    {
        ...
        implementedHooks_[1] = IPolicyManager
            .PolicyHook
            .PostCallOnExternalPosition;
        implementedHooks_[2] = IPolicyManager
            .PolicyHook
    ...
    function validateRule(
        ...
        if (_hook == IPolicyManager.PolicyHook.CreateExternalPosition) {
            ...
            ) = __decodeCreateExternalPositionValidationData(_encodedArgs);
        } else if (
            _hook == IPolicyManager.PolicyHook.PostCallOnExternalPosition
        ) {
            ...
            ) = __decodePostCallOnExternalPositionValidationData(_encodedArgs);
            externalPositionTypeId = IExternalPositionProxy(externalPosition)
                .getExternalPositionType();
        } else if (
            _hook == IPolicyManager.PolicyHook.ReactivateExternalPosition
        ) {
            address externalPosition;
            (
                caller,
                externalPosition
            ) = __decodeReactivateExternalPositionValidationData(_encodedArgs);
            externalPositionTypeId = IExternalPositionProxy(externalPosition)
                .getExternalPositionType();
        } else {
            ...
            (
                caller,
                externalPosition
            ) = __decodeRemoveExternalPositionValidationData(_encodedArgs);
            externalPositionTypeId = IExternalPositionProxy(externalPosition)
                .getExternalPositionType();
        }
    ...
    function passesRule(
        ...
        uint256[] memory listIds = getListIdsForFundAndUser(
            _comptrollerProxy,
            _caller
        );
        ...
        return
            UINT_LIST_REGISTRY_CONTRACT.isInSomeOfLists(
                listIds,

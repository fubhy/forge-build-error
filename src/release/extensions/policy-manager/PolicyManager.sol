    ...
    GasRelayRecipientMixin
{
    ...
    mapping(address => mapping(PolicyHook => address[]))
        private comptrollerProxyToHookToPolicies;

    modifier onlyFundOwner(address _comptrollerProxy) {
        require(
            __msgSender() ==
                IVault(getVaultProxyForFund(_comptrollerProxy)).getOwner(),
    ...
    function activateForFund(bool _isMigratedFund) external override {
        ...
        if (_isMigratedFund) {
            address[] memory enabledPolicies = getEnabledPoliciesForFund(
                comptrollerProxy
            );
        ...
        onlyFundOwner(_comptrollerProxy)
    {
        ...
        for (uint256 i; i < implementedHooks.length; i++) {
            ...
            if (disabled) {
                emit PolicyDisabledOnHookForFund(
                    _comptrollerProxy,
                    _policy,
                    implementedHooks[i]
                );
    ...
    function __activatePolicyForFund(address _comptrollerProxy, address _policy)
        private
    {
    ...
    function __enablePolicyForFund(
        ...
        for (uint256 i; i < _hooks.length; i++) {
            ...
            comptrollerProxyToHookToPolicies[_comptrollerProxy][_hooks[i]].push(
                    _policy
                );
        }
    ...
    function policyIsEnabledOnHookForFund(
        ...
        return
            getEnabledPoliciesOnHookForFund(_comptrollerProxy, _hook).contains(
                _policy
            );
    }
}

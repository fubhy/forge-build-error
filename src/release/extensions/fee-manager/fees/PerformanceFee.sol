...
contract PerformanceFee is FeeBase, UpdatableFeeRecipientBase {
  ...
  function activateForFund(address _comptrollerProxy, address)
    external
    override
    onlyFeeManager
  {
    ...
    uint256 grossSharePrice = ComptrollerLib(_comptrollerProxy)
      .calcGrossShareValue();
  ...
  function addFundSettings(
    ...
    (uint256 feeRate, address recipient) = abi.decode(
      _settingsData,
      (uint256, address)
    );
    ...
    require(
      feeRate <= ONE_HUNDRED_PERCENT,
      "addFundSettings: feeRate max exceeded"
    );
  ...
  {
    uint256 sharePrice;
    (sharePrice, sharesDue_) = __calcSharesDue(
      _comptrollerProxy,
      _vaultProxy,
      _gav
    );
  ...
  function update(
    ...
    if (
      comptrollerProxyToFeeInfo[_comptrollerProxy].highWaterMark ==
      RESET_HWM_FLAG
    ) {
  ...
  function __calcGrossShareValueForComptrollerProxy(
    ...
    if (_sharesSupply == 0) {
      return
        10 **
          uint256(
            ERC20(ComptrollerLib(_comptrollerProxy).getDenominationAsset())
              .decimals()
          );
    }

...
abstract contract ExitRateFeeBase is FeeBase {
  ...
  ) public virtual override onlyFeeManager {
    (uint16 inKindRate, uint16 specificAssetsRate) = abi.decode(
      _settingsData,
      (uint16, uint16)
    );
    require(
      inKindRate < ONE_HUNDRED_PERCENT,
      "addFundSettings: inKindRate max exceeded"
    );
    require(
    ...
    )
  {
    ...
    (
      ...
      sharesRedeemed,
      forSpecificAssets
    ) = __decodePreRedeemSharesSettlementData(_settlementData);
  ...
  function getInKindRateForFund(address _comptrollerProxy)
    public
    view

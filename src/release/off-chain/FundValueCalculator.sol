...
contract FundValueCalculator is IFundValueCalculator {
  ...
  {
    (address denominationAsset, uint256 valueInDenominationAsset) = calcGav(
      _vaultProxy
    );
    ...
    returns (uint256 grossShareValue_)
  {
    ...
    ) = calcGrossShareValue(_vaultProxy);
  ...
  {
    (address denominationAsset, uint256 valueInDenominationAsset) = calcNav(
      _vaultProxy
    );
  ...
  {
    ComptrollerLib comptrollerProxyContract = __getComptrollerProxyForVault(
      _vaultProxy
    );
  ...
  function calcNetValueForSharesHolder(
    address _vaultProxy,
    address _sharesHolder
  ) public override returns (address denominationAsset_, uint256 netValue_) {
    ...
    returns (uint256 sharesDue_)
  {
    ...
    uint256 lastPaid = ProtocolFeeTracker(getProtocolFeeTracker())
      .getLastPaidForVault(_vaultProxy);
    ...
    uint256 rawSharesDue = sharesSupply
      .mul(
        ProtocolFeeTracker(getProtocolFeeTracker()).getFeeBpsForVault(
          _vaultProxy
        )
      )
      .mul(secondsDue)

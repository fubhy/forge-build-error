  ...
  GasRelayRecipientMixin
{
  ...
  function __assertIsGasRelayPaymaster() private view {
    require(
      msg.sender == getGasRelayPaymaster(),
      "Only Gas Relay Paymaster callable"
    );
  }

  function __assertIsOwner(address _who) private view {
    require(
      _who == IVault(getVaultProxy()).getOwner(),
  ...
  function __assertSharesActionNotTimelocked(
    ...
    uint256 lastSharesBoughtTimestamp = getLastSharesBoughtTimestampForAccount(
      _account
    );

    require(
      lastSharesBoughtTimestamp == 0 ||
        block.timestamp.sub(lastSharesBoughtTimestamp) >=
  ...
  function callOnExtension(
    ...
    IExtension(_extension).receiveCallFromComptroller(
      __msgSender(),
      _actionId,
      _callArgs
    );
  ...
  function __buyBackMaxProtocolFeeShares(address _vaultProxy, uint256 _gav)
    private
  {
    uint256 sharesAmount = ERC20(_vaultProxy).balanceOf(
      getProtocolFeeReserve()
    );
    uint256 buybackValueInMln = __getBuybackValueInMln(
      _vaultProxy,
      sharesAmount,
      _gav
    );
    ...
    try
      IVault(_vaultProxy).buyBackProtocolFeeShares(
        sharesAmount,
        buybackValueInMln,
        _gav
      )
    {} catch (bytes memory reason) {
      emit BuyBackMaxProtocolFeeSharesFailed(
        reason,
        sharesAmount,
        buybackValueInMln,
        _gav
  ...
  function __getBuybackValueInMln(
    ...
    uint256 buybackValueInDenominationAsset = grossShareValue
      .mul(_sharesAmount)
      .div(SHARES_UNIT);
  ...
  function init(address _denominationAsset, uint256 _sharesActionTimelock)
    external
    override
  {
    require(getDenominationAsset() == address(0), "init: Already initialized");
    require(
      IValueInterpreter(getValueInterpreter()).isSupportedPrimitiveAsset(
        _denominationAsset
      ),
  ...
  function setVaultProxy(address _vaultProxy)
    external
    override
    onlyFundDeployer
  {
  ...
  function destructActivated(
    ...
    try
      IVault(getVaultProxy()).payProtocolFee{ gas: _payProtocolFeeGasLimit }()
    {} catch {
    ...
    try
      IExtension(getFeeManager()).deactivateForFund{
        gas: _deactivateFeeManagerGasLimit
      }()
    {} catch {
  ...
  function calcGav() public override returns (uint256 gav_) {
    ...
    if (externalPositions.length > 0) {
      for (uint256 i; i < externalPositions.length; i++) {
        uint256 externalPositionValue = __calcExternalPositionValue(
          externalPositions[i]
        );
  ...
  function buySharesOnBehalf(
    ...
    require(
      !hasSharesActionTimelock ||
        IFundDeployer(getFundDeployer()).isAllowedBuySharesOnBehalfCaller(
          canonicalSender
        ),
    ...
    returns (uint256 sharesReceived_)
  {
    ...
    require(
      ...
      "__buyShares: _minSharesQuantity must be >0"
    );
    ...
    uint256 sharesIssued = receivedInvestmentAmount.mul(SHARES_UNIT).div(
      sharePrice
    );
    ...
    sharesReceived_ = ERC20(vaultProxyCopy).balanceOf(_buyer).sub(
      prevBuyerShares
    );
    require(
  ...
  function __parseRedemptionPayoutAssets(
    ...
    address[] memory trackedAssetsToPayout = _trackedAssets.removeItems(
      _assetsToSkip
    );
    ...
    payoutAssets_ = new address[](
      trackedAssetsToPayout.length.add(additionalItemsCount)
    );
  ...
  function preTransferSharesHook(
    ...
    address vaultProxyCopy = getVaultProxy();
    require(
      msg.sender == vaultProxyCopy,
      "preTransferSharesHook: Only VaultProxy callable"
    );
  ...
  function preTransferSharesHookFreelyTransferable(address _sender)
    external
    view
    override
  {
  ...
  function deployGasRelayPaymaster() external onlyOwnerNotRelayable {
    ...
    address paymaster = IBeaconProxyFactory(getGasRelayPaymasterFactory())
      .deployProxy(constructData);
  ...
  function pullWethForGasRelayer(uint256 _amount)
    external
    override
    onlyGasRelayPaymaster
  {
    IVault(getVaultProxy()).withdrawAssetTo(
      getWethToken(),
  ...
  function getIntegrationManager()
    ...
    view
    override
    returns (address integrationManager_)
  {
  ...
  function getPolicyManager()
    ...
    view
    override
    returns (address policyManager_)
  {
  ...
  function getProtocolFeeReserve()
    public
    view
    returns (address protocolFeeReserve_)
  {
  ...
  function doesAutoProtocolFeeSharesBuyback()
    public
    view
    returns (bool doesAutoBuyback_)
  {
  ...
  function getSharesActionTimelock()
    public
    view
    returns (uint256 sharesActionTimelock_)
  {

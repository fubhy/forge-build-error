...
contract VaultLib is VaultLibBase2, IVault, GasRelayRecipientMixin {
  ...
  {
    return
      IExternalPositionManager(getExternalPositionManager())
        .getExternalPositionLibForType(_typeId);
  }
  ...
  function setFreelyTransferableShares() external onlyOwner {
    require(
      ...
      "setFreelyTransferableShares: Already set"
    );
  ...
  function setSymbol(string calldata _nextSymbol) external override {
    require(
      __msgSender() == owner || msg.sender == getFundDeployer(),
      "Unauthorized"
    );
  ...
  function addAssetManagers(address[] calldata _managers) external onlyOwner {
    for (uint256 i; i < _managers.length; i++) {
      require(
        !isAssetManager(_managers[i]),
        "addAssetManagers: Manager already registered"
      );
  ...
  function removeAssetManagers(address[] calldata _managers)
    external
    onlyOwner
  {
    for (uint256 i; i < _managers.length; i++) {
      require(
        isAssetManager(_managers[i]),
        "removeAssetManagers: Manager not registered"
      );
  ...
  function setAccessorForFundReconfiguration(address _nextAccessor)
    external
    override
  {
    require(
      msg.sender == getFundDeployer(),
  ...
  function __executeVaultActionAddExternalPosition(bytes memory _actionData)
    private
  {
  ...
  function __executeVaultActionAddTrackedAsset(bytes memory _actionData)
    private
  {
  ...
  function __executeVaultActionApproveAssetSpender(bytes memory _actionData)
    private
  {
  ...
  function __executeVaultActionCallOnExternalPosition(bytes memory _actionData)
    private
  {
    (
      ...
      address[] memory assetsToReceive
        ...
        _actionData,
        (address, bytes, address[], uint256[], address[])
      );
  ...
  function __executeVaultActionRemoveExternalPosition(bytes memory _actionData)
    private
  {
  ...
  function __executeVaultActionRemoveTrackedAsset(bytes memory _actionData)
    private
  {
  ...
  function __executeVaultActionTransferShares(bytes memory _actionData)
    private
  {
  ...
  function __executeVaultActionWithdrawAssetTo(bytes memory _actionData)
    private
  {
    (address asset, address target, uint256 amount) = abi.decode(
      _actionData,
  ...
  function __validatePositionsLimit() private view {
    require(
      trackedAssets.length + activeExternalPositions.length <
        getPositionsLimit(),
  ...
  function __invokePreTransferSharesHook(
    ...
    if (sharesAreFreelyTransferable()) {
      ...
      IComptroller(accessor).preTransferSharesHook(
        _sender,
        _recipient,
        _amount
      );
  ...
  function canManageAssets(address _who)
    external
    view
    override
    returns (bool canManageAssets_)
  {
  ...
  function canRelayCalls(address _who)
    external
    view
    override
    returns (bool canRelayCalls_)
  {
  ...
  function getTrackedAssets()
    ...
    view
    override
    returns (address[] memory trackedAssets_)
  {
  ...
  function isAssetManager(address _who)
    public
    view
    returns (bool isAssetManager_)
  {
  ...
  function isTrackedAsset(address _asset)
    public
    view
    override
    returns (bool isTrackedAsset_)
  {

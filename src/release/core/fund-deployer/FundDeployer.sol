    ...
    GasRelayRecipientMixin
{
    ...
    mapping(bytes32 => mapping(bytes32 => bool))
        private vaultCallToPayloadToIsAllowed;
    mapping(address => ReconfigurationRequest)
    ...
    function __assertIsMigrator(address _vaultProxy, address _who)
        private
        view
    {
        require(
    ...
    function setVaultLib(address _vaultLib)
        external
        onlyOwner
        pseudoConstant(getVaultLib())
    {
        vaultLib = _vaultLib;
    ...
    function setGasLimitsForDestructCall(
        ...
        require(
            _nextDeactivateFeeManagerGasLimit > 0 &&
                _nextPayProtocolFeeGasLimit > 0,
    ...
    function setReleaseLive() external {
        ...
        require(
            ...
            "setReleaseLive: comptrollerLib is not set"
        );
        require(
        ...
        require(
            ...
            "setReleaseLive: vaultLib is not set"
        );
    ...
    function __destructActivatedComptrollerProxy(address _comptrollerProxy)
        private
    {
        (
        ...
        returns (address comptrollerProxy_, address vaultProxy_)
    {
        ...
        vaultProxy_ = __deployVaultProxy(
            _fundOwner,
            comptrollerProxy_,
            _fundName,
            _fundSymbol
        ...
        IProtocolFeeTracker(getProtocolFeeTracker()).initializeForVault(
            vaultProxy_
        );
    ...
    function createReconfigurationRequest(
        ...
        require(
            IDispatcher(getDispatcher()).getFundDeployerForVaultProxy(
                _vaultProxy
            ) == address(this),
        ...
        uint256 executableTimestamp = block.timestamp +
    ...
    function __deployComptrollerProxy(
        ...
        comptrollerProxy_ = address(
            new ComptrollerProxy(constructData, getComptrollerLib())
        );
    ...
    {
        address nextComptrollerProxy = vaultProxyToReconfigurationRequest[
            _vaultProxy
    ...
    {
        ReconfigurationRequest
            memory request = getReconfigurationRequestForVaultProxy(
                _vaultProxy
            );
        require(
        ...
        address paymaster = IComptroller(prevComptrollerProxy)
            .getGasRelayPaymaster();
        __destructActivatedComptrollerProxy(prevComptrollerProxy);

        // Execute the reconfiguration
        IVault(_vaultProxy).setAccessorForFundReconfiguration(
        ...
        if (paymaster != address(0)) {
            IComptroller(request.nextComptrollerProxy).setGasRelayPaymaster(
                paymaster
            );
        }
    ...
    function setReconfigurationTimelock(uint256 _nextTimelock)
        external
        onlyOwner
    {
    ...
    function cancelMigration(
        ...
        IDispatcher(getDispatcher()).cancelMigration(
            _vaultProxy,
            _bypassPrevReleaseFailure
        );
    }
    ...
    function executeMigration(
        ...
        dispatcherContract.executeMigration(
            _vaultProxy,
            _bypassPrevReleaseFailure
        );
    ...
    function deregisterBuySharesOnBehalfCallers(address[] calldata _callers)
        external
        onlyOwner
    {
    ...
    function registerBuySharesOnBehalfCallers(address[] calldata _callers)
        external
        onlyOwner
    {
        for (uint256 i; i < _callers.length; i++) {
    ...
    function deregisterVaultCalls(
        ...
        require(
            _contracts.length == _selectors.length &&
                _contracts.length == _dataHashes.length,
    ...
    function isAllowedVaultCall(
        ...
        bytes32 contractFunctionHash = keccak256(
    ...
    function getGasLimitsForDestructCall()
        ...
        view
        returns (
            uint256 deactivateFeeManagerGasLimit_,
            uint256 payProtocolFeeGasLimit_
        )
    {
        return (
    ...
    function getProtocolFeeTracker()
        public
        view
        returns (address protocolFeeTracker_)
    {
    ...
    function getReconfigurationTimelock()
        public
        view
        returns (uint256 reconfigurationTimelock_)
    {
    ...
    function isRegisteredVaultCall(
        ...
        return
            vaultCallToPayloadToIsAllowed[
                keccak256(abi.encodePacked(_contract, _selector))
            ][_dataHash];
    }

  ...
  MathHelpers
{
  ...
  function calcCanonicalAssetsTotalValue(
    ...
    for (uint256 i; i < _baseAssets.length; i++) {
      uint256 assetValue = __calcAssetValue(
        _baseAssets[i],
        _amounts[i],
        _quoteAsset
      );
  ...
  function calcCanonicalAssetValue(
    ...
    if (isSupportedPrimitiveAsset(_quoteAsset)) {
      ...
      isSupportedDerivativeAsset(_quoteAsset) &&
      isSupportedPrimitiveAsset(_baseAsset)
    ) {
  ...
  {
    return
      isSupportedPrimitiveAsset(_asset) || isSupportedDerivativeAsset(_asset);
  }
  ...
  function __calcPrimitiveToDerivativeValue(
    ...
    uint256 derivativeUnit = 10 **
      uint256(ERC20(_derivativeQuoteAsset).decimals());

    address derivativePriceFeed = getPriceFeedForDerivative(
  ...
  function removePrimitives(address[] calldata _primitives)
    external
  ...
  function setEthUsdAggregator(address _nextEthUsdAggregator)
    external
    onlyFundDeployerOwner
  {
    ...
    returns (bool isSupported_)
  {
    return
      _asset == getWethToken() ||
      getAggregatorForPrimitive(_asset) != address(0);
  }

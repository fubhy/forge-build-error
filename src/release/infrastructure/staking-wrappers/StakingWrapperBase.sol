...
abstract contract StakingWrapperBase is
  IStakingWrapper,
  ERC20,
  ReentrancyGuard
{
  ...
  event TotalHarvestIntegralUpdated(
  ...
  mapping(address => mapping(address => UserHarvestData))
    private rewardTokenToUserToHarvestData;
  ...
  function withdrawToOnBehalf(
    ...
    _approve(
      _onBehalf,
      msg.sender,
      allowance(_onBehalf, msg.sender).sub(_amount)
    );
  ...
  function __calcClaimableRewardForIntegralDiff(
    ...
    return
      balanceOf(_account)
        .mul(_totalHarvestIntegral.sub(_userHarvestIntegral))
        .div(INTEGRAL_PRECISION);
  }
    ...
    returns (address[] memory rewardTokens_, uint256[] memory claimedAmounts_)
  {
    ...
    for (uint256 i; i < rewardTokens_.length; i++) {
      claimedAmounts_[i] = __updateHarvestAndClaim(
        rewardTokens_[i],
        _account,
        supply
      );
  ...
  function __updateHarvest(
    ...
    TotalHarvestData storage totalHarvestData = rewardTokenToTotalHarvestData[
      _rewardToken
    ];
    ...
    for (uint256 i; i < _accounts.length; i++) {
      ...
      UserHarvestData storage userHarvestData = rewardTokenToUserToHarvestData[
        _rewardToken
      ][_accounts[i]];

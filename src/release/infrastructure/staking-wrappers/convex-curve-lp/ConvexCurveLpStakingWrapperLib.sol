...
contract ConvexCurveLpStakingWrapperLib is StakingWrapperLibBase {
    ...
    function init(uint256 _pid) external {
        ...
        IConvexBooster.PoolInfo memory poolInfo = CONVEX_BOOSTER_CONTRACT
            .poolInfo(_pid);
        ...
        __setTokenName(
            string(
                abi.encodePacked(
                    "Enzyme Staked: ",
                    ERC20(poolInfo.token).name()
                )
            )
        );
        __setTokenSymbol(
            string(abi.encodePacked("stk", ERC20(poolInfo.token).symbol()))
        );
    ...
    function addExtraRewards() public {
        IConvexBaseRewardPool convexPoolContract = IConvexBaseRewardPool(
            getConvexPool()
        );
        ...
        for (uint256 i; i < extraRewardsCount; i++) {
            ...
            __addRewardToken(
                IConvexVirtualBalanceRewardPool(
                    convexPoolContract.extraRewards(i)
                ).rewardToken()
            );
        }
    }
    ...
    function setApprovals() public {
        ERC20(getCurveLpToken()).safeApprove(
            address(CONVEX_BOOSTER_CONTRACT),
            type(uint256).max
        );
    }
    ...
    function __depositLogic(address _from, uint256 _amount) internal override {
        ERC20(getCurveLpToken()).safeTransferFrom(
            _from,
            address(this),
            _amount
        );
    ...
    function __withdrawLogic(address _to, uint256 _amount) internal override {
        IConvexBaseRewardPool(getConvexPool()).withdrawAndUnwrap(
            _amount,
            false
        );

...
contract ConvexCurveLpStakingWrapperFactory is BeaconProxyFactory {
    ...
    function deploy(uint256 _pid) external returns (address wrapperProxy_) {
        require(
            getWrapperForConvexPool(_pid) == address(0),
            "deploy: Wrapper already exists"
        );
        ...
        address lpToken = ConvexCurveLpStakingWrapperLib(wrapperProxy_)
            .getCurveLpToken();
    ...
    function getCurveLpTokenForWrapper(address _wrapper)
        external
        view
    ...
    function getWrapperForConvexPool(uint256 _pid)
        public
        view

...
abstract contract ChainlinkPriceFeedMixin {
  ...
  event EthUsdAggregatorSet(
    address prevEthUsdAggregator,
    address nextEthUsdAggregator
  );
  ...
  function __calcCanonicalValue(
    ...
    require(
      quoteAssetRate > 0,
      "__calcCanonicalValue: Invalid quote asset rate"
    );
  ...
  function __calcConversionAmount(
    ...
    (
      ,
      int256 ethPerUsdRate,
      ,
      uint256 ethPerUsdRateLastUpdatedAt,
  ...
  function __calcConversionAmountEthRateAssetToUsdRateAsset(
    ...
    uint256 intermediateStep = _baseAssetAmount
      .mul(_baseAssetRate)
      .mul(_ethPerUsdRate)
      .div(ETH_UNIT);

    return
      intermediateStep.mul(_quoteAssetUnit).div(_baseAssetUnit).div(
        _quoteAssetRate
      );
  }
  ...
  function __calcConversionAmountUsdRateAssetToEthRateAsset(
    ...
    return
      intermediateStep.mul(ETH_UNIT).div(_baseAssetUnit).div(_quoteAssetRate);
  }
    ...
    returns (int256 rate_)
  {
    ...
    (, rate_, , rateUpdatedAt, ) = IChainlinkAggregator(aggregator)
      .latestRoundData();
  ...
  function getEthUsdAggregator()
    public
    view
    returns (address ethUsdAggregator_)
  {
  ...
  function getUnitForPrimitive(address _primitive)
    public
    view
    returns (uint256 unit_)
  {
    if (_primitive == getWethToken()) {
      return ETH_UNIT;
    }

...
contract CurvePriceFeed is IDerivativePriceFeed, FundDeployerOwnerMixin {
  ...
  event InvariantProxyAssetForPoolSet(
  ...
  event ValidatedVirtualPriceForPoolUpdated(
    address indexed pool,
    ...
    returns (address[] memory underlyings_, uint256[] memory underlyingAmounts_)
  {
    ...
    if (poolInfo.invariantProxyAssetDecimals == 18) {
      underlyingAmounts_[0] = _derivativeAmount.mul(virtualPrice).div(
        VIRTUAL_PRICE_UNIT
      );
    } else {
  ...
  function addPools(
    ...
    for (uint256 i; i < _pools.length; i++) {
      ...
      if (_lpTokens[i] == registryContract.get_lp_token(_pools[i])) {
        ...
        _lpTokens[i] == _pools[i] && factoryContract.get_n_coins(_pools[i]) > 0
      ) {
  ...
  function removeDerivatives(address[] calldata _derivatives)
    external
  ...
  function removePools(address[] calldata _pools)
    external
    onlyFundDeployerOwner
  {
    for (uint256 i; i < _pools.length; i++) {
  ...
  function setCurvePoolOwner(address _nextPoolOwner)
    external
    onlyFundDeployerOwner
  {
  ...
  function __addDerivative(address _derivative, address _pool) private {
    ...
    require(
      ERC20(_derivative).decimals() == 18,
      "__addDerivative: Not 18-decimal"
    );
  ...
  function __addGaugeTokens(
    address[] calldata _gaugeTokens,
    address[] calldata _pools
  ) private {
    require(
      _gaugeTokens.length == _pools.length,
      "__addGaugeTokens: Unequal arrays"
    );
  ...
  function __getRegistryMainContract()
    private
    view
    returns (ICurveRegistryMain contract_)
  {
    ...
    returns (ICurveRegistryMetapoolFactory contract_)
  {
    return
      ICurveRegistryMetapoolFactory(
        ADDRESS_PROVIDER_CONTRACT.get_address(
          ADDRESS_PROVIDER_METAPOOL_FACTORY_ID
        )
      );
  }
  ...
  function __setPoolInfo(
    ...
    if (_reentrantVirtualPrice) {
      ...
      emit ValidatedVirtualPriceForPoolUpdated(
        _pool,
        lastValidatedVirtualPrice
      );
    }
  ...
  function __updateValidatedVirtualPrice(address _pool, uint256 _virtualPrice)
    private
  {
    // Validate the virtual price by calling a non-reentrant pool function
    __makeNonReentrantPoolCall(_pool);
  ...
  function getLpTokenForPool(address _pool)
    public
    view
    returns (address lpToken_)
  {
  ...
  function getPoolInfo(address _pool)
    public
    view
    returns (PoolInfo memory poolInfo_)
  {
  ...
  function getPoolForDerivative(address _derivative)
    public
    view

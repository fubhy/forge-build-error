...
abstract contract CompoundPriceFeedBase is
  ...
  FundDeployerOwnerMixin
{
    ...
    returns (address[] memory underlyings_, uint256[] memory underlyingAmounts_)
  {
    ...
    require(
      ...
      "calcUnderlyingValues: Unsupported derivative"
    );
  ...
  function addCEtherTokens(address[] calldata _cEtherTokens)
    external
  ...
  function addCTokens(address[] calldata _cTokens)
    external
    onlyFundDeployerOwner
  {
    ...
    for (uint256 i; i < _cTokens.length; i++) {
      require(
        cTokenToToken[_cTokens[i]] == address(0),
        "addCTokens: Value already set"
      );
  ...
  function getTokenFromCToken(address _cToken)
    public
    view

  ...
  UniswapV2PoolTokenValueCalculator
{
    ...
    returns (address[] memory underlyings_, uint256[] memory underlyingAmounts_)
  {
    ...
    (
      uint256 token0TrustedRateAmount,
      uint256 token1TrustedRateAmount
    ) = __calcTrustedRate(
  ...
  function __calcTrustedRate(
    ...
    if (
      IValueInterpreter(VALUE_INTERPRETER).isSupportedPrimitiveAsset(_token0)
    ) {
      token1RateAmount_ = 10**_token1Decimals;
      token0RateAmount_ = ValueInterpreter(VALUE_INTERPRETER)
        .calcCanonicalAssetValue(_token1, token1RateAmount_, _token0);
    } else {
      token0RateAmount_ = 10**_token0Decimals;
      token1RateAmount_ = ValueInterpreter(VALUE_INTERPRETER)
  ...
  function addPoolTokens(address[] calldata _poolTokens)
    external
  ...
  {
    IValueInterpreter valueInterpreterContract = IValueInterpreter(
      VALUE_INTERPRETER
    );
  ...
  {
    return (
      poolTokenToInfo[_poolToken].token0,
      poolTokenToInfo[_poolToken].token1
    );
  }

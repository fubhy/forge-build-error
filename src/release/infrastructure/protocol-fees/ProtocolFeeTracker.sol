...
contract ProtocolFeeTracker is IProtocolFeeTracker, FundDeployerOwnerMixin {
  ...
  event FeeBpsOverrideSetForVault(
    address indexed vaultProxy,
    uint256 nextFeeBpsOverride
  );

  event FeePaidForVault(
    address indexed vaultProxy,
  ...
  function getFeeBpsForVault(address _vaultProxy)
    public
    view
    returns (uint256 feeBps_)
  {
  ...
  function __setLastPaidForVault(address _vaultProxy, uint256 _nextTimestamp)
    private
  {
  ...
  function setFeeBpsDefault(uint256 _nextFeeBpsDefault)
    external
  ...
  function setFeeBpsOverrideForVault(
    address _vaultProxy,
    uint256 _nextFeeBpsOverride
  ) external onlyFundDeployerOwner {
    require(
      _nextFeeBpsOverride < MAX_BPS,
      "setFeeBpsOverrideForVault: Exceeds max"
    );
    ...
    onlyFundDeployerOwner
  {
    ...
    require(
      prevTimestamp > 0,
      "setLastPaidForVault: _vaultProxy not initialized"
    );
    require(
  ...
  function getLastPaidForVault(address _vaultProxy)
    public
    view
    returns (uint256 lastPaid_)
  {

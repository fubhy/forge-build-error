...
contract GasRelayPaymasterLib is IGasRelayPaymaster, GasRelayPaymasterLibBase1 {
  ...
  function init(address _vault) external {
    require(
      getParentVault() == address(0),
      "init: Paymaster already initialized"
    );
  ...
  function postRelayedCall(
    ...
    (address spender, bytes4 selector) = abi.decode(
      _context,
      (address, bytes4)
    );
  ...
  function withdrawBalance() external override {
    ...
    IGsnRelayHub(getHubAddr()).withdraw(
      getRelayHubDeposit(),
  ...
  function getParentComptroller()
    public
    view
    returns (address parentComptroller_)
  {
  ...
  function __depositMax() private {
    ...
    if (prevDeposit < DEPOSIT) {
      ...
      IGasRelayPaymasterDepositor(getParentComptroller()).pullWethForGasRelayer(
          amount
        );
  ...
  function __isAllowedCall(
    ...
    if (_contract == parentComptroller) {
      ...
      _contract == ComptrollerLib(parentComptroller).getPolicyManager()
    ) {
      if (
        ...
        return
          __parseTxDataFirstParameterAsAddress(_txData) ==
          getParentComptroller();
      }
    } else if (
      _contract == ComptrollerLib(parentComptroller).getFundDeployer()
    ) {
      if (
        ...
        return
          __parseTxDataFirstParameterAsAddress(_txData) == getParentVault();
      }
    }
  ...
  function getRelayHubDeposit()
    ...
    view
    override
    returns (uint256 depositBalance_)
  {
  ...
  function trustedForwarder()
    ...
    view
    override
    returns (address trustedForwarder_)
  {
  ...
  function versionPaymaster()
    ...
    view
    override
    returns (string memory versionString_)
  {

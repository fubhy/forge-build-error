...
contract GlobalConfigLib is IGlobalConfig1, GlobalConfigLibBase1 {
    ...
    function isValidRedeemSharesCall(
        ...
        address fundDeployer = IDispatcher(getDispatcher())
        ...
        if (fundDeployer == FUND_DEPLOYER_V4) {
            ...
            if (
                _redeemContract !=
                IGlobalConfigVaultAccessGetter(_vaultProxy).getAccessor()
            ) {
                return false;
            }
            ...
            (address encodedRecipient, uint256 encodedSharesAmount) = abi
                .decode(_redeemData, (address, uint256));

...
contract ExternalPositionFactory {
    ...
    function addNewPositionTypes(string[] calldata _labels)
        external
        onlyDispatcherOwner
    {
    ...
    function updatePositionTypeLabels(
        uint256[] calldata _typeIds,
        string[] calldata _labels
    ) external onlyDispatcherOwner {
        require(
            _typeIds.length == _labels.length,
            "updatePositionTypeLabels: Unequal arrays"
        );
    ...
    function addPositionDeployers(address[] memory _accounts)
        external
        onlyDispatcherOwner
    {
    ...
    function removePositionDeployers(address[] memory _accounts)
        external
        onlyDispatcherOwner
    {
        for (uint256 i; i < _accounts.length; i++) {
    ...
    function getPositionTypeCounter()
        public
        view
        returns (uint256 positionTypeCounter_)
    {
    ...
    function isPositionDeployer(address _account)
        public
        view
        returns (bool isPositionDeployer_)
    {

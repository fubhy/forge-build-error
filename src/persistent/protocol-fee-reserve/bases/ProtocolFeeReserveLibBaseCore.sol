...
abstract contract ProtocolFeeReserveLibBaseCore is
    ProxiableProtocolFeeReserveLib
{
    ...
    function init(address _dispatcher) external {
        require(
            getDispatcher() == address(0),
            "init: Proxy already initialized"
        );
    ...
    function getProtocolFeeReserveLib()
        public
        view
        returns (address protocolFeeReserveLib_)
    {
        assembly {

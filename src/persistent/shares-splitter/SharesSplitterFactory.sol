...
contract SharesSplitterFactory {
    ...
    constructor(address _globalConfigProxy) public {
        SHARES_SPLITTER = address(
            new SharesSplitterLib(_globalConfigProxy, address(this))
        );
    }
    ...
    function deploy(
    ...
    ) external returns (address sharesSplitter_) {
        require(
            _users.length == _splitPercentages.length,
            "deploy: Unequal arrays"
        );
        ...
        sharesSplitter_ = address(
            new SharesSplitterProxy(constructData, SHARES_SPLITTER)
        );

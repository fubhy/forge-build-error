...
abstract contract TreasurySplitterMixin {
  ...
  mapping(address => mapping(address => uint256))
    private userToTokenToBalClaimed;
  ...
  function claimToken(address _token)
    external
    virtual
    ...
    returns (uint256 balClaimable_)
  {
    return
      __calcTokenBalClaimable(
        ...
        getTotalTokenBalClaimed(_token).add(
  ...
  function __claimTokenWithoutTransfer(
    ...
    uint256 totalCumulativeBal = totalBalClaimed.add(
      ERC20(_token).balanceOf(address(this))
    );
    ...
    if (_amount == type(uint256).max) {
      ...
      require(
        _amount <= claimableBalForUser,
        "claimToken: _amount exceeds claimable"
      );
    ...
    userToTokenToBalClaimed[_user][_token] = balClaimedForUser.add(
      claimedAmount_
    );
  ...
  function __setSplitRatio(
    ...
    require(
      totalSplitPercentage == ONE_HUNDRED_PERCENT,
      "__setSplitRatio: Split not 100%"
    );
  }

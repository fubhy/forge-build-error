...
contract MockGenericExternalPositionLib is IExternalPosition {
    ...
    function receiveCallFromVault(bytes memory _actionData) external override {
        ...
        if (
            actionId ==
            uint256(MockGenericExternalPositionActions.AddManagedAssets)
        ) {
            __addManagedAssets(assets, amounts);
        } else if (
            actionId ==
            uint256(MockGenericExternalPositionActions.RemoveManagedAssets)
        ) {
            __removeManagedAssets(assets);
        } else if (
            actionId ==
            uint256(MockGenericExternalPositionActions.AddDebtAssets)
        ) {
            __addDebtAssets(assets, amounts);
        } else if (
            actionId ==
            uint256(MockGenericExternalPositionActions.RemoveDebtAssets)
        ) {

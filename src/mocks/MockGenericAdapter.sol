...
contract MockGenericAdapter is AdapterBase {
  ...
  {
    if (
      _selector == bytes4(keccak256("swapDirectFromVault(address,bytes,bytes)"))
    ) {
      return IIntegrationManager.SpendAssetsHandleType.None;
    }
    if (
      _selector == bytes4(keccak256("swapViaApproval(address,bytes,bytes)"))
    ) {
  ...
  function swapA(
    ...
    fundAssetsTransferHandler(
      ...
      __getSpendAssetsHandleTypeForSelector(
        bytes4(keccak256("swapA(address,bytes,bytes)"))
      )
    )
  {
  ...
  function swapB(
    ...
    fundAssetsTransferHandler(
      ...
      _assetTransferArgs,
      __getSpendAssetsHandleTypeForSelector(
        bytes4(keccak256("swapB(address,bytes,bytes)"))
      )
    )
  {
    __decodeCallArgsAndSwap(_callArgs);
